...
$row1 = row_array($sql1);
		...
		$sql1 = "SELECT * FROM tb_course_class a INNER JOIN tb_course_class_detail b ON a.course_class_id = b.course_class_id INNER JOIN tb_course_class_level c ON b.course_class_detail_id=c.course_class_detail_id WHERE a.course_class_id='{$course_cid}' AND c.teacher_id2 = '{$tid}' AND a.classroom_t_id='$clid' AND b.subject_id = '{$sid}' AND a.course_class_type='$typec'";
		...
		foreach ($list as $key => $row) { 
			...
			$sqlScor2 = "SELECT * FROM tb_score a INNER JOIN tb_score_detail b ON a.score_id = b.score_id WHERE a.course_class_id = '{$course_cid}' AND a.classroom_t_id='{$clid}' AND a.term_id = '{$check_term}' AND a.grade_id = '{$check_grade}' AND a.subject_id = '{$sid}' AND a.teacher_id = '{$tid}' AND a.score_type='2' AND a.score_no = '$exam_no' AND a.score_status='1' AND b.course_s_id = '$course_s_id' AND b.course_class_detail_id = '$course_class_detail_id' AND b.course_class_level_id='$course_class_level_id' AND b.score_type='2' ";
		...
		//echo "<meta charset='utf-8'/><script>alert('Save data successfully.!!');location.href='../?modules=check_subject_teach_detail&id=$sid&idd=$idd&tid=$tid&cdid=$cdid&cid=$cid&rid=$clid&exam_no=$exam_no&typec=$typec&check_term=$check_term&check_grade=$check_grade';</script>";
