...
$now_date = date('Y-m-d');
		...
		if($_FILES["filegrade"]["name"]!="") {
				...
				if ($file_ext1 == ".pdf" || $file_ext1 == ".PDF") {
					$filename = date('YmdHis') . "_grade" . $file_ext1;

					if (move_uploaded_file($_FILES["filegrade"]["tmp_name"], "../../checkgrade/uploads/filegrade/" . $filename)) {
		...
		echo "<meta charset='utf-8'/><script>alert('แก้ไขข้อมูลสำเร็จ!!');location.href='../?modules=report_show_grade1&classroom=$classroom&check_year=$check_year';</script>";
		//echo "<meta charset='utf-8'/><script>alert('เพิ่มข้อมูลสำเร็จ!!');window.location.href = document.referrer;</script>";

		?modules=report_show_grade1&classroom=147&check_year=2562
