...
<script src="<?php echo $RunLink->Call_Link_System();?>/template/global_assets/js/plugins/extensions/jquery_ui/interactions.min.js"></script>
    ...
    <script>
        $(document).ready(function(){
            $('#changepass_cancel').on('click',function(){
                var changepass_cancel=$("#changepass_cancel").val();
                    if(changepass_cancel==="changepass_cancel"){
                        document.location="<?php echo $RunLink->Call_Link_System();?>/?modules=changepass"
                    }else{}
            })
        })
    </script>
                            ...
                            if(action==""){
                                ...
                                if(password!=""){
                                    if(password.length>=6){
                                        if(password.match(/^([0-9])+$/i)){
                                            ...
                                            '<input type="password" name="password" id="password" class="form-control is-invalid" value="'+password+'" placeholder="ข้อมูล Password " required="required">'+
                                            '<span class="invalid-feedback">ต้องมีตัวเลขและตัวอังกฤษ</span'
                                            password_error="yes";
                                        }else{
                                            document.getElementById("password-null").innerHTML=
                                            '<input type="password" name="password" id="password" class="form-control" value="'+password+'" placeholder="ข้อมูล Password " required="required">'+
                                            '<span class="invalid-feedback">ข้อมูล Password </span>'
                                            password_error="no";
                                        }
                                    }else{
                                        document.getElementById("password-null").innerHTML=
                                    ...
                                    document.getElementById("password-null").innerHTML=
                                    '<input type="password" name="password" id="password" class="form-control is-invalid" value="" placeholder="ข้อมูล Password " required="required">'+
                                    '<span class="invalid-feedback">กรุณาข้อมูล Password </span>'
                                    password_error="yes";
                                }
                                ...
                                if(password2!=""){
                                    if(password===password2){
                                            ...
                                            '<input type="password" name="password2" id="password2" class="form-control" value="'+password2+'" placeholder="ข้อมูล Password" required="required">'+
                                            '<span class="invalid-feedback">ข้อมูล Password ถูกต้อง</span>'
                                            password_error="no";
                                    }else{
                                        document.getElementById("password2-null").innerHTML=
                                        '<input type="password" name="password2" id="password2" class="form-control is-invalid" value="'+password2+'" placeholder="ข้อมูล Password" required="required">'+
                                        '<span class="invalid-feedback">ข้อมูล Password ไม่ถูกต้อง</span>'
                                        password_error="yes";
                                    }
                                }else{
                                    document.getElementById("password2-null").innerHTML=
