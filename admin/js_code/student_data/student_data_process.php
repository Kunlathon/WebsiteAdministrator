...
if (($action == "create")) {
    ...
    if ((filter_input(INPUT_POST, 'nickname') != null)) {
        ...
        $row_ter = row_array($sql_ter);
            ...
            if ($check) {
                ...
                //echo "<meta charset='utf-8'/><script>alert('เพิ่มข้อมูลสำเร็จ!!');location.href='../?modules=student_data2';</script>";
                echo "no_error";
            } else {
    ...
    $teacher_key = filter_input(INPUT_POST, 'id');

    delete($table, "{$ff} = '{$teacher_key}'");

    echo "no_error";

   //echo "table->".$table."ff->".$ff."teacher_key->".$teacher_key; 
