...
<!--Add-->
<script>
	var ABA_AddAlumniData = function () {
            var _componentABA_AddAlumniData = function() {
                if (typeof swal == 'undefined') {
                    console.warn('Warning - sweet_alert.min.js is not loaded.');
                    return;
                }
// Defaults
                var swalInitAddAlumniData = swal.mixin({
...
//--------------------------------------------------------------------------------------
//--------------------------------------------------------------------------------------
		$('#Add_Alumni_data').on('click', function() {
            swalInitAddAlumniData.fire({
                ...
                if(result.value) {
                    ...
                    var student_key=$("#student_key").val();
                        ...
                        if(action==""){
                            ...
                            if(classroom!="" &&  grade!=""){
                                $.post("<?php echo $RunLink->Call_Link_System();?>/js_code/alumni_data/alumni_data_process.php",{
                                    action:action,
                                    classroom:classroom,
                                    grade:grade,
                                            ...
                                            swalInitAddAlumniData.fire({
                                                ...
                                                didOpen: function() {
                                                    ...
                                                    timerInterval = setInterval(function() {
                                                        ...
                                                        if (content) {
                                                            const b_alumni_data = content.querySelector('b_alumni_data')
                                                            if (b_alumni_data) {
                                                                b_alumni_data.textContent = Swal.getTimerLeft();
                                                            }else{}
                                                        }else{}
                                                    }, 100);
                                                },
                                                ...
                                                if (result.dismiss === Swal.DismissReason.timer) {
                                                    document.location="<?php echo $RunLink->Call_Link_System();?>?modules=student_alumni"; 
                                                }else{}
                                            });
...
//--------------------------------------------------------------------------------------
        return {
            initComponentsAddAlumniData: function() {
                _componentABA_AddAlumniData();
            }
        }
    }();
// Initialize module
// ------------------------------
    document.addEventListener('DOMContentLoaded', function() {
        ABA_AddAlumniData.initComponentsAddAlumniData();
    });
</script>
<!--Add End-->
                ...
                if (result.value) {
                    ...
                    if (action == "") {
                        ...
                        if (name != "" && ename_error != "yes" && esurname_error != "yes" && username != "" && grade != "" && classroom != "" && status != "") {
                            $.post("<?php echo $RunLink->Call_Link_System(); ?>/js_code/student_alumni/student_alumni_process.php", {
                                action: action,
                                idcard: idcard,
                                name: name,
                                surname: surname,
