...
<div class="table-responsive">
        <table class="table table-bordered" id="datatable-button-html5-columns-STD" style="width: 100%;">
            ...
            <tbody>
    <?php
            $news_sql = "SELECT * FROM `tb_news`  WHERE `news_category_id`='3'  ORDER BY `news_id` DESC";
                    ...
                    <td align="center" style="width: 5%; vertical-align: text-top;" class="align-top">
                        <div align="center">
                            <ul class="nav justify-content-center">
                                <li class="nav-item">
<form name="news_update<?php echo $news_row["news_id"];?>" accept-charset="utf-8" method="post" action="<?php echo $RunLink->Call_Link_System(); ?>/?modules=procurement_news">
                ...
                swalInitDeteleImageData.fire({
                    ...
                    if(result.value){
                        ...
                        if(action_error=="no_error" && news_id_error=="no_error"){
                            $.post("<?php echo $RunLink->Call_Link_System();?>/js_code/procurement_news/procurement_news_process.php",{
                                action:action,
                                news_id:news_id
                                    ...
                                    let timerInterval;
                                        swalInitDeteleImageData.fire({
                                            ...
                                            if (result.dismiss === Swal.DismissReason.timer) {
                                                document.location = "<?php echo $RunLink->Call_Link_System(); ?>/?modules=procurement_news";
                                            } else {}
                                        });
