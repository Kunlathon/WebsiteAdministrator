...
if ((is_null(filter_input(INPUT_POST, 'check_year')))) {
    ...
    $check_year = filter_input(INPUT_POST, 'class_name');
    $sql = "SELECT * 
                    FROM `tb_term` 
...
<script>
    $(document).ready(function() {
        $('#check_year').on('change', function() {
            ...
            if ((cy_key != "")) {
                $.post("<?php echo $RunLink->Call_Link_System(); ?>/js_code/report_score/report_score_classroom1.php", {
                    cy_key: cy_key,
                    cg_key: cg_key
                }, function(run_ct) {
                    if (run_ct != "") {
...
<script>
    $(document).ready(function() {
        $('#classroom').on('change', function() {
            ...
            if ((check_year != "" && check_classroom != "" && check_grade != "" && class_name != "")) {
                $.post("<?php echo $RunLink->Call_Link_System(); ?>/js_code/report_score/report_score_table1.php", {
            ...
            <fieldset class="mb-3">
                <div class="form-group row">
                    ...
                    <div class="col-<?php echo $grid; ?>-9">
                        <select class="form-control select-search" name="check_year" id="check_year" data-placeholder="เลือกปีการศึกษา..." required="required">
                            <option></option>
                            <optgroup label="ปีการศึกษา">
                                ...
                                ?>
                                    <option value="<?php echo $_cc['year']; ?>" <?php echo $cc_selected; ?>><?php echo $_cc['year']; ?></option>
                                <?php } ?>
                            </optgroup>
                        </select>
                    </div>
